<?php
	class general extends datebase{
		...
		public function listper(){
			$query = $this->consulta("SELECT id_per, nombre_per, cod_per FROM personal_mant ORDER BY nombre_per ASC");
			if($this->numero_de_filas($query) > 0){
				while ( $tsArray = $this->fetch_assoc($query) ) {
					$data[] = $tsArray;
				}
		...
		public function repMes($a,$b){
			$query = $this->consulta("SELECT turno_per, id_check, codigo_check, tipo_check, dia_check, hor_check,notas_check, nombre_per,verifica_check,fechcon_check FROM check_mant
									INNER JOIN personal_mant ON codigo_check = cod_per
									WHERE mes_check = '$a[mes]' AND anio_check = '$a[anio]'".$a['fal']." AND fechcon_check <= '$b'
									ORDER BY turno_per, nombre_per,dia_check,tipo_check ");
			if($this->numero_de_filas($query) > 0){
				while ( $tsArray = $this->fetch_assoc($query) ) {
					$data[] = $tsArray;
		...
		public function newEvent($value){
			$this->consulta("INSERT INTO eventos_mant (nombre_eve,fecha_eve,descripcion_eve,archivo_eve,num_ofi_eve)
								VALUES ('$value[nom_eve]','$value[fecha_eve]','$value[des]','$value[foto]','$value[numero]')");
		}
		public function getEventosDate($fecha){
			$query = $this->consulta("SELECT * FROM eventos_mant WHERE fecha_eve = '$fecha'");
			if($this->numero_de_filas($query)>0){
				while($datos = $this->fetch_assoc($query))
					$data[] = $datos;
				return $data;
