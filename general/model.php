<?php
	class general extends datebase{
		...
		public function listper(){
			$query = $this->consulta("SELECT id_per, nombre_per, cod_per FROM personal_mant where status_per in (0,1,3,5) ORDER BY nombre_per ASC");
			if($this->numero_de_filas($query) > 0){
				while ( $tsArray = $this->fetch_assoc($query) ) {
					$data[] = $tsArray;
				}
